...
import MovieCard from "../components/MovieCard"; // путь зависит от структуры
import { Modal, Button } from "react-bootstrap"; // Убедись, что установлен bootstrap и react-bootstrap
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="d-flex flex-wrap">
        {movies.map((movie) => (
          <MovieCard key={movie.id} movie={movie} onClick={setSelectedMovie} addToCart={addToCart} />
        ))}
      </div>
      ...
      {selectedMovie && (
        <Modal show={true} onHide={() => setSelectedMovie(null)}>
          <Modal.Header closeButton>
            <Modal.Title>{selectedMovie.title} ({selectedMovie.releaseYear})</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <img src={selectedMovie.imageUrl} className="img-fluid mb-3" alt={selectedMovie.title} />
